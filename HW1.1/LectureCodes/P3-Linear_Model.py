...
HW1.1, Part-III: Regression Workflow (Linear)
09/07/2021
"""
...
class Data:
    ...
    def plot_loss(self, iterations, loss_train, loss_val):
        ...
        # add a title
        plt.title('Training & Validation Loss for Linear Model')
    ...
    def plot_model(self,
        ...
        plt.plot(child_df, y_hat_all_orig, 'k-', label = 'Model')
        
        # add a title
        plt.title('Linear Model Predicting Child Weight')
        
        # specify child age on the x-axis
        plt.xlabel(data_obj.xlab)
        
        # specify child weight on the y-axis
    ...
    def plot_parity(self,
        ...
        # add a title
        plt.title('Parity Plot of Actual and Predicted Weight (Linear)')
        
        # specify actual child weight values on the x-axis
        plt.xlabel('Actual Weight [lb]')
        
        # specify predicted child weight on the y-axis
...
# extract only the customers that are children
children = customer_df[customer_df['Age_Class'] == 0]

# drop the age column as it is no longer needed since all rows are children
child_df = children.drop('Age_Class', axis = 1)

# normalize the input and output variables using StandardScaler()
scaled = StandardScaler().fit_transform(child_df)
...
# define a linear model function that inputs x and p vectors and outputs y
def model_func(x, p):
    
    # return y as a linear combo of input age vector and current parameters
    return p[0]*x + p[1]
...
def loss(p):
    ...
    # make predictions using the linear model with train and current params
    y_hat_train = model_func(x_train, p)
    
    # make predictions using the linear model with val and current params
...
p0 = np.random.uniform(0.5, 1, size = 2)
...
# print the optimal slope and intercept of the linear weight(age) model
...
# save the slope parameter to a variable
m = popt[0]

# save the intercept parameter to a variable
b = popt[1]

# use the model to make predictions with the train set
y_hat_train = m*x_train + b
...
x_train_orig = np.std(child_df['Age'])*x_train + np.mean(child_df['Age'])
x_val_orig = np.std(child_df['Age'])*x_val + np.mean(child_df['Age'])

# convert actual values and predicted values from train back to original space
y_train_orig = np.std(child_df['Weight'])*y_train + np.mean(child_df['Weight'])
y_hat_train_orig = np.std(child_df['Weight'])*y_hat_train + np.mean(child_df['Weight'])

# convert actual and predicted values from validation back to original space
y_val_orig = np.std(child_df['Weight'])*y_val + np.mean(child_df['Weight'])
y_hat_val_orig = np.std(child_df['Weight'])*y_hat_val + np.mean(child_df['Weight'])

# convert all predicted values back to original space
y_hat_all_orig = np.std(child_df['Weight'])*y_hat_all + np.mean(child_df['Weight'])
...
Data().plot_model(x_train_orig,    # unnormalized age training data
                  ...
                  child_df['Age'], # unnormalized age data (all)
